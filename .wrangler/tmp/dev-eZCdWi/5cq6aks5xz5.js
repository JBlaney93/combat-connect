// .wrangler/tmp/bundle-iZzEG1/checked-fetch.js
...
// .wrangler/tmp/pages-Fabg1D/bundledWorker-0.6287978072267943.mjs
  ...
  async set(e, r) {
    ...
    switch (await this.update(e, JSON.stringify(s)), s.value?.kind) {
      case "FETCH": {
        ...
        await this.setTags(a, { cacheKey: e }), A(a).forEach((n) => N.delete(n));
      }
    }
  }
  async get(e) {
    ...
    switch (s.value?.kind) {
      case "FETCH":
        return await this.loadTagsManifest(), A(s.value.data.tags ?? []).some((i) => {
          if (N.has(i))
            return true;
...
var E = class {
  ...
  getLocaleFriendlyRoute(e, r) {
    return !this.locales || r !== "miss" ? e : z(e.src, this.locales) ? { ...e, src: e.src.replace(/\/\(\.\*\)\$$/, "(?:/(.*))?$") } : e;
  }
  ...
  async checkPhase(e) {
    ...
    let a = "miss";
    return s || e === "miss" || e === "error" ? a = "hit" : r && (a = H(e)), this.checkPhase(a);
  }
  async run(e = "none") {
...
async function Q(t, e, r) {
  let s = new E(e.routes, r, t, e.wildcard), a = await X(s);
  return _e(t, a, r);
}
async function X(t, e = "none", r = false) {
  return await t.run(e) === "error" || !r && t.status && t.status >= 400 ? X(t, "error", true) : { path: t.path, status: t.status, headers: t.headers, searchParams: t.searchParams, body: t.body };
}
...
var Dt = { async fetch(t, e, r) {
  ...
  return s.run({ ...e, NODE_ENV: "production", SUSPENSE_CACHE_URL: w }, async () => {
    ...
    let n = B(t);
    return Q({ request: n, ctx: r, assetsFetcher: e.ASSETS }, p, f);
  });
} };
...
// .wrangler/tmp/pages-Fabg1D/5cq6aks5xz5.js
...
// .wrangler/tmp/bundle-iZzEG1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-iZzEG1/middleware-loader.entry.ts
